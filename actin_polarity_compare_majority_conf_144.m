...
load('/home/Medalia/Projects7/Bruno/ActinPolarity/mapping/mapping_144/bundles/all_bundles_distributions_144.mat','spin_filament_conf_tomo_all','spin_filament_conf_all');
...
figure;figure_indx = gcf;figure(figure_indx);plot(bundle_dir_all,spin_filament_conf_all,'r+');
...
for k=1:7
    ...
    bundle_dir_conf(1,k) = mean(spin_filament_conf_all(indx_sel));
    bundle_dir_conf_std(1,k) = std(spin_filament_conf_all(indx_sel));
...
ylabel('Majority confidence score');
...
figure;figure_indx = gcf;figure(figure_indx);histogram(spin_filament_conf_all,'BinEdges',[0.5 0.6 0.7 0.8 0.9 1.0]);
xlabel('Majority confidence score');
ylabel('Number of rec. filaments');
ylim([0 1230]);

% Number and percentage of filaments above threshold
size(find(spin_filament_conf_all>=0.60),1)
size(find(spin_filament_conf_all>=0.60),1)./size(find(spin_filament_conf_all>=0.0),1)
% Length threshold = 3 ---> ~92% (2679 of 2893 filaments) of all filaments with a length of at least 3 segments are equal or better this score
